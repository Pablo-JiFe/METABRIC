...
# that recieved chemotherapy and with recurrence parameters



# 1.- Preparing metadata --------------------------------------------------

er_patients_nochem <- metadata %>% 
...
# 1.3 Object with all the patients ER+ and chmeo + and expression of only the genes of interest

late_genes.patients <- counts_data[late_death.genes, er_patients_nochem$PATIENT_ID]
...
all(rownames(late_genes.patients) == er_patients_nochem$PATIENT_ID)
...
late_genes.patients <- 
  late_genes.patients %>% 
  as.data.frame() %>% 
  mutate(SURVIVAL = er_patients_nochem$RECURR_STAT, 
         SURVIVAL = as.numeric(SURVIVAL),
         SURVIVAL_MON = er_patients_nochem$RFS_MONTHS,
