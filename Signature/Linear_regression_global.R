...
text_signature <- paste0(
  label,
  "se evaluo una firma con regresión lineal penalizada con un alpha de ",
  best_params$mixture,
  " un lambda de ",
  best_params$penalty,
...
late_genes.patients.cox <- 
  ...
  mutate(HER2 = HER2_SNP6, 
         ...
         SCORE = test_pred$.pred_linear_pred * - 1,
...
text_independent_prog <- 
  paste0(
    "Al realizar un cox multivariado junto a edad, tratamiento, ganglios linfaticos y menopausia, la firma ",
    if((independent_prog[independent_prog$term == "SCORE",]$p.value < 0.05) == TRUE){
